...
public class SearchProductServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Product> list = dao.searchProductByNameNDescribe(key, key);

        PrintWriter out = response.getWriter();

        for (Product product : list) {
            String shortDescribe = product.getDescribe();
            if (shortDescribe.length() > 10) {
                shortDescribe = shortDescribe.substring(0, 10) + "...";
            }
            ...
            out.println("<tr><td>"
                    ...
                    + "<button onclick=\"choose('" + product.getId() + "')\">Choose</button>"
                    + "</td>\n"
                    + "</tr>");
        }
    }
